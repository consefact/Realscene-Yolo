...
`config.yaml` 里 `synth.target_types.<类型>` 决定自动标注怎么画框，三种方式：

| 方式 | 含义 | 适用场景 |
|---|---|---|
| `roi: null` | **整张目标图就是检测框** | 目标裁剪图 / 渲染图，整图即物体 |
| `roi: [rx, ry, rw, rh]` | 目标是"底板"，**只框内部 ROI**（相对目标框的比例） | 物体贴在一块底板/靶标上，只想框物体本身 |
| `label_dir: <目录>` | **打框 alpha 解耦**：与 dir 同布局/同画布的另一份"打框 alpha"图，标注紧框取自该图 alpha | 底板固定、中心换图；**透视/旋转（单应变换）下框仍贴合内容**（`roi` 比例法会漂移） |

一个 target_type = 一个来源目录 + 一种标注方式 + 一段缩放范围。可以配多个类型，realscene 会全部加载混合使用。
增强功能的完整清单与用法见 **[docs/augmentation.md](docs/augmentation.md)**。
